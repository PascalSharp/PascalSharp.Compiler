...
namespace PascalSharp.Internal.Debugger.Threads
{
	...
	{	
		...
		public ISymUnmanagedMethod symMethod {
			get	{
				if (symReader == null) {
					return null;
				} else {
		...
		private DebugType declaringType;
		...
		public DebugType DeclaringType
		{
			get
			{
				...
				declaringType = DebugType.Create(process,ContaingClass);
		...
		NamedValueCollection fieldsCache;
		
		public NamedValueCollection Fields {
			get
			{
				if (fieldsCache == null)
				{
					fieldsCache = new NamedValueCollection(FieldsEnum);
				}
				return fieldsCache;
			}
		}
		...
		IEnumerable<NamedValue> FieldsEnum {
			get
			{
				if (symMethod != null)
				{
					ISymUnmanagedScope symRootScope = symMethod.RootScope;
					IList<FieldInfo> flds = DeclaringType.GetFields(BindingFlags.All);
					foreach (FieldInfo fi in flds)
						if (!fi.Name.Contains("$") && fi.IsStatic)
