...
namespace PascalSharp.Internal.Lexer
{
    ...
    namespace PascalLanguage
    {
        public partial class GPPGParser
        {
            ...
            {
                return new LexLocation(sc.begin_position.line_num, sc.begin_position.column_num - 1,
                    sc.end_position.line_num, sc.end_position.column_num, sc.FileName);
            }
            ...
            public typecast_node NewAsIsConstexpr(expression constterm, op_typecast typecastop, type_definition tdef,
                LexLocation loc)
            {
                var naic = new typecast_node(constterm as addressed_value, tdef, typecastop, loc);
                if (!(constterm is addressed_value))
                    parsertools.errors.Add(new bad_operand_type(parsertools.CurrentFileName, constterm.source_context,
                        naic));
                return naic;
            }
            ...
            public method_name NewQualifiedIdentifier(method_name qualified_identifier, ident identifier,
                LexLocation loc)
            {
            ...
            public simple_property NewSimplePrimPropertyDefinition(simple_property simple_property_definition,
                LexLocation loc)
            {
            ...
            public property_accessors NewPropertySpecifiersRead(ident tkRead, ident opt_identifier,
                property_accessors property_specifiers, LexLocation loc)
            {
                ...
                else
                {
                    if (opt_identifier != null)
                        nnps.read_accessor = new read_accessor_name(opt_identifier,
                            tkRead.source_context.Merge(opt_identifier.source_context));
            ...
            public property_accessors NewPropertySpecifiersWrite(ident tkWrite, ident opt_identifier,
                property_accessors property_specifiers, LexLocation loc)
            {
                ...
                if (opt_identifier != null)
                    nnpsw.write_accessor = new write_accessor_name(opt_identifier,
                        tkWrite.source_context.Merge(opt_identifier.source_context));
            ...
            public while_node NewWhileStmt(token_info tkWhile, expression expr, token_info opt_tk_do, statement stmt,
                LexLocation loc)
            {
            ...
            public statement_list NewLambdaBody(expression expr_l1, LexLocation loc)
            {
                ...
                parsertools.create_source_context(ass, id,
                    expr_l1); // дурацкая функция - если хотя бы у одного sc=null, то возвращает null
